...
- [⚠️ إعداد n8n 2.0+](#️-إعداد-n8n-20-مطلوب)
...
**سير عمل n8n** — أتمتة مرئية بدون كود:
- روبوت Telegram: أرسل صورة → الذكاء الاصطناعي يتعرف → يُرجع تقدير التكاليف
- أتمتة البريد الإلكتروني: استلم BOQ → معالجة → إرسال تقدير منسق
- خط أنابيب BIM: Revit → Excel → DDC CWICR → تقرير 5D

**Dify / LangChain** — بناء تطبيقات LLM:
- ربط DDC CWICR كقاعدة معرفة
- تكوين خط أنابيب RAG مع تضمينات OpenAI
- إنشاء روبوت دردشة لتقدير التكاليف
...
|----------------|-------|
| **التقدير الفوري** | احصل على تقديرات التكاليف من الأوصاف النصية أو الصور |
| **توليد BOQ** | إنشاء جداول الكميات تلقائياً |
| **مقارنة الأسعار** | مقارنة التكاليف عبر المناطق |
| **تخطيط الموارد** | حساب ساعات العمل والمواد والمعدات |
| **تكامل BIM** | تقدير 4D/5D آلي من Revit/IFC |
| **تدريب نماذج AI** | استخدام البيانات المنظمة لضبط الذكاء الاصطناعي الإنشائي |
...
### ✅ متطلب أساسي مهم (الإصدار 17 وما بعده)

بدءًا من **الإصدار 17**، تستخدم المحولات خط أساس تشغيل محدث.
يرجى تثبيت **Microsoft Visual C++ Redistributable 2015–2022 (x64)** قبل تشغيل أي أدوات `.exe` (`RvtExporter.exe`، `RVT2IFCconverter.exe`، إلخ).

- التحميل الرسمي (Microsoft Learn): **أحدث VC++ 2015–2022 مدعوم**.
- اختر حزمة **x64** وشغل `VC_redist.x64.exe`.

> بدون هذه الحزمة، ستفشل بعض الأنظمة (خاصة تثبيتات Windows الجديدة/الأجهزة الافتراضية) في تشغيل المحولات.

**لماذا تحتاج هذا:** قام الإصدار 17 بتغيير خط الأساس التقني من الإصدار 16؛ وقت تشغيل VC++ هو تبعية مطلوبة الآن.
...
## ⚠️ إعداد n8n 2.0+ مطلوب

> **بدءًا من إصدار n8n 2.0، تم تعطيل عقدة Execute Command افتراضيًا لأسباب أمنية.**
>
> بدون الإعداد أدناه، لن تعمل سير العمل التي تستخدم Execute Command — ستظهر العقد بعلامة استفهام أو لن يتم التعرف عليها.

### الإصلاح السريع

**Windows (CMD) — شغل في كل مرة:**
...
> 📚 المزيد من التفاصيل: [تغييرات n8n 2.0 المكسرة](https://docs.n8n.io/2-0-breaking-changes/)
